# Chapter9.仮想マシンをバックアップしてみよう

## 参考サイト

- [Azure Backup の価格](https://azure.microsoft.com/ja-jp/pricing/details/backup/)

- [Azure Backup のインスタント リストア機能を使用してバックアップと復元のパフォーマンスを改善する](https://learn.microsoft.com/ja-jp/azure/backup/backup-instant-restore-capability)

- [スナップショットの整合性](https://learn.microsoft.com/ja-jp/azure/backup/backup-azure-vms-introduction#snapshot-consistency)

- [Azure Backup の バックアップ / リストア 所要時間について](https://jpabrs-scem.github.io/blog/AzureBackupGeneral/Backup_RecoveryTIme/)

- [Azure 仮想マシンのバックアップからファイルを回復する](https://learn.microsoft.com/ja-jp/azure/backup/backup-azure-restore-files-from-vm)

- [Azure Backup の論理的な削除](https://learn.microsoft.com/ja-jp/azure/backup/backup-azure-security-feature-cloud)


