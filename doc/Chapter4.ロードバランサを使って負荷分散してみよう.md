...
以下を入力して「追加」をクリック。**一番下のSNATのチェックボックスを「既定の送信アクセス」に変更するのを忘れないように注意。**
- 名前：lb-rule（任意）
- IPバージョン：IPv4
...
- TCPリセット：無効
- フローティングIP：無効
- アウトバウンドの送信元ネットワーク アドレス変換 (SNAT)：既定の送信アクセスを使用します

![負荷分散規則2](https://user-images.githubusercontent.com/22112831/187810717-3e24c1da-90b0-43d1-9274-c66298453e6e.png)
...
「34. 負荷分散の動作確認」の動画の手順を参考にしてください。

## 6.インバウンドNAT規則の追加

インバウンドNAT規則の概要（仕組み）については「35. インバウンドNAT規則の追加」の動画を参考にしてください。
